...
import { Readiness } from '@/classes/Readiness.class';
import { type IKvStore, type KV, type KvOptions, StorageType, DiscardPolicy } from '../types';

// Class
// ===========================================================

export class KvCommon extends Client {
    ...
    private async _createOrGetStore(storeName: string, options?: Partial<KvOptions>): Promise<void> {
        ...
        const kvOptions = {
            ...
            max_bytes: 500 * 1024 * 1024,   // The maximum number of bytes on the KV
            ttl: 60 * 60 * 1000,            // The maximum number of millis the key should live in the KV
            storage: StorageType.Memory,    // The storage type for the KV
            history: 1,                     // The number of revisions to keep in the Kv
            discard: DiscardPolicy.Old,     // The policy for discarding old revisions
            // Additional options
            ...(options || {}),
        }
